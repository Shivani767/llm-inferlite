...
class PagedAttentionCache(KVCacheStrategy):
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        ...
        return {
            "used_pct": (used_blocks / self.total_blocks) * 100 if self.total_blocks else None,
            "total_blocks": self.total_blocks,
            "free_blocks": self.free_blocks,
            "note": "Block accounting only. Fragmentation percent is not measured here.",
        }
...
class PrefixCache(KVCacheStrategy):
    ...
    def get_memory_usage(self) -> Dict[str, float]:
        return {"saved_memory_mb": None, "cached_prefixes": len(self.cached_prefixes), "note": "prefix table size only; MB savings are not estimated"}
