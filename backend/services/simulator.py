...
class DistributedInferenceSimulator:
    ...
    async def run_simulation(self) -> DistributedSimulationResult:
        ...
        # Utilization proxy from occupancy of the concurrency limiter — not NVML.
        occupancy = min(1.0, self.max_seen_concurrency / max(1, self.config.concurrency_limit))
        gpu_util = [round(100.0 * occupancy, 2) for _ in range(self.config.num_gpus)]
